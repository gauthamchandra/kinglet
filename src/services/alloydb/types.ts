...
// Both enums post-date the shipped `@google-cloud/alloydb` protos, so their wire
// numbers are inferred from the discovery document's declaration order rather than
// read off a proto. Each is a flat, sequential proto3 enum with its default value
// at 0, so the mapping is unambiguous — flagged in the PR all the same.
const DATA_API_ACCESS_ENUM: EnumNumberMap = {
  0: 'DEFAULT_DATA_API_ENABLED_FOR_GOOGLE_CLOUD_SERVICES',
  1: 'DISABLED',
  2: 'ENABLED',
};

const MAINTENANCE_VERSION_SELECTION_POLICY_ENUM: EnumNumberMap = {
  0: 'MAINTENANCE_VERSION_SELECTION_POLICY_UNSPECIFIED',
  1: 'MAINTENANCE_VERSION_SELECTION_POLICY_LATEST',
  2: 'MAINTENANCE_VERSION_SELECTION_POLICY_DEFAULT',
};
...
const CLUSTER_ENUM_FIELDS: Readonly<Record<string, EnumNumberMap>> = {
  ...
  maintenanceVersionSelectionPolicy: MAINTENANCE_VERSION_SELECTION_POLICY_ENUM,
};
...
const INSTANCE_ENUM_FIELDS: Readonly<Record<string, EnumNumberMap>> = {
  ...
  dataApiAccess: DATA_API_ACCESS_ENUM,
};
...
/**
 * Data-plane credentials for an AlloyDB user.
 *
 * <p>An empty stored password skips the wire challenge (Cloud SQL's "no
 * password required"). AlloyDB built-in users never allow that: an omitted
 * `initialUser` still has to fail login. `loginDisabled` challenges and then
 * rejects every password, rather than substituting a guessable sentinel.
 */
export function alloydbDataPlaneUser(stored: string): {
  password: string;
  loginDisabled?: boolean;
} {
  return stored === '' ? { password: '', loginDisabled: true } : { password: stored };
}
