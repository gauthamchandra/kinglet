...
beforeEach(async () => {
  ...
  userHandlers = new UserHandlers(new UserService(users, clusters), responseUtils);
...
describe('route ids', () => {
  ...
  test('supportedDatabaseFlagsList_isRegisteredBeforeTheLocationGetThatCouldShadowIt', () => {
    const ids = locationHandlers.getRoutes().map(route => route.id);

    expect(ids.indexOf('alloydb.supportedDatabaseFlags.list')).toBeLessThan(
      ids.indexOf('alloydb.locations.get')
    );
  });
});
...
describe('location handlers', () => {
  test('list_returnsLocationsScopedToTheRequestedProject', async () => {
    const response = await invoke(locationHandlers.getRoutes(), 'alloydb.locations.list', {
      params: { project: PROJECT },
    });

    const locations = body(response).locations as Array<{ name: string; locationId: string }>;

    expect(response.status).toBe(200);
    expect(locations.length).toBeGreaterThan(0);
    expect(locations[0]?.name).toStartWith('projects/p/locations/');
  });

  test('get_returnsTheRequestedLocation', async () => {
    const response = await invoke(locationHandlers.getRoutes(), 'alloydb.locations.get');

    expect(response.status).toBe(200);
    expect(body(response).locationId).toBe(LOCATION);
    expect(body(response).name).toBe('projects/p/locations/us-central1');
  });

  test('get_givenAnUnservedRegion_returns404', async () => {
    const response = await invoke(locationHandlers.getRoutes(), 'alloydb.locations.get', {
      params: { project: PROJECT, location: 'mars-central1' },
    });
...
describe('static list pagination', () => {
  /**
   * Both `locations.list` and `supportedDatabaseFlags.list` declare
   * `pageSize`/`pageToken` in the discovery document. Serving the whole constant
   * array regardless would make a client that pages hang or double-process.
   */
  test('locationsList_honoursPageSizeAndReturnsANextPageToken', async () => {
    const response = await invoke(locationHandlers.getRoutes(), 'alloydb.locations.list', {
      params: { project: PROJECT },
      query: { pageSize: '3' },
    });

    expect((body(response).locations as unknown[]).length).toBe(3);
    expect(body(response).nextPageToken).toBe('3');
  });

  test('locationsList_followingTheTokenContinuesWithoutRepeating', async () => {
    const firstPage = await invoke(locationHandlers.getRoutes(), 'alloydb.locations.list', {
      params: { project: PROJECT },
      query: { pageSize: '3' },
    });
    const secondPage = await invoke(locationHandlers.getRoutes(), 'alloydb.locations.list', {
      params: { project: PROJECT },
      query: { pageSize: '3', pageToken: '3' },
    });

    const firstNames = (firstPage.body as { locations: { name: string }[] }).locations.map(
      location => location.name
    );
    const secondNames = (secondPage.body as { locations: { name: string }[] }).locations.map(
      location => location.name
    );

    expect(secondNames).toHaveLength(3);
    expect(firstNames.some(name => secondNames.includes(name))).toBe(false);
  });

  test('locationsList_onTheFinalPage_omitsNextPageToken', async () => {
    const response = await invoke(locationHandlers.getRoutes(), 'alloydb.locations.list', {
      params: { project: PROJECT },
      query: { pageSize: '1000' },
    });
