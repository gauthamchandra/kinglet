...
- [ ] **All commits are signed off under my identity.** Humans use `git commit -s`. Cloud agents keep their own git identity so GitHub can verify the signature, and put me on `Co-authored-by` and `Signed-off-by`. By signing off I certify the [DCO](https://developercertificate.org/) — I have the right to submit this under Apache-2.0 — **and that I have read and stand behind every line of it.** An agent may apply my sign-off only after I have reviewed and approved the change; CI rejects sign-offs from coding-agent identities.
